...
export default function Fromaddkhohang({onClose,isAdding, setIsAdding,selectedProduct}) {
    ...
    const handleSubmit=async (event)=>{
        ...
        var res = await axios.post('https://localhost:7084/api/Warehouses',inputvalue)
        console.log(res)
        if(res.status === 201){
    ...
    const handleSubmitUpdate = async (event) => {
      ...
      try {
        ...
        const res = await axios.put(`https://localhost:7084/api/Customers/${inputvalue.id}`, inputvalue);
        console.log(res);
        if (res.status === 204) {
          alert('Bạn đã sửa Khách hànhg thành công');
        } else {
          alert(res.data.error);
        }
      } catch (error) {
